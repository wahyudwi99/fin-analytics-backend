...
async def get_user_data(request: Request,
                        credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer())):
    """
    Get user data based on the email from cookie session
    """
    token_bearer = credentials.credentials
    if str(token_bearer) != BACKEND_API_SECRET_KEY:
        raise HTTPException(status_code=403, detail="Authorization was failed !")
...
async def update_user_profile(json_data: dict,
                              credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer())):
    """
    ...
    try:
        await asyncio.to_thread(update_user_profile_data, json_data)
        json_result = {
            "status": 200,
